...
class MainContainer extends Component {
  ...
  getThreads = async () => {
    const threads = await fetch("http://srvvrs7.herokuapp.com/threads/", {
      method: "GET"
    });
  ...
  addThread = async (thread, e) => {
    ...
    try {
      const createThread = await fetch(
        "http://srvvrs7.herokuapp.com/threads/",
        {
          method: "POST",
  ...
  deleteThread = async (id, e) => {
    ...
    try {
      const deleteThread = await fetch(
        "http://srvvrs7.herokuapp.com/threads/" + id,
        {
          method: "DELETE",
      ...
      if (parsedResponse.status === 204) {
        this.setState({
          threads: this.state.threads.filter((threads, i) => threads.id !== id)
        });
        this.props.history.push({
          pathname: "/home/",
          state: { threadPotato: "" }
  ...
  getPosts = async () => {
    const posts = await fetch("http://srvvrs7.herokuapp.com/posts/", {
      method: "GET"
    });
  ...
  addPost = async (post, e) => {
    ...
    try {
      const createPost = await fetch("http://srvvrs7.herokuapp.com/posts/", {
        method: "POST",
  ...
  deletePost = async (id, e) => {
    ...
    try {
      const deletePost = await fetch(
        "http://srvvrs7.herokuapp.com/posts/" + id,
        {
          method: "DELETE"
        }
      );
      ...
      if (parsedResponse.status === 204) {
        this.setState({
          posts: this.state.posts.filter((posts, i) => posts.id !== id)
        });
      } else {
        console.log("There is a problem with delete");
      }
  ...
  closeAndEdit = async e => {
    ...
    try {
      const editResponse = await fetch(
        "http://srvvrs7.herokuapp.com/posts/" + this.state.editPostId,
        {
          method: "PUT",
        ...
        <div>
          ...
          <Route
            ...
            render={props => {
              return (
                <div>
                  <CreateThread {...props} addThread={this.addThread} />
                </div>
              );
            }}
          />
